...
@section('container')
<form action="/addtocart" method="POST" id="scheduleForm">
    @csrf
    <input type="hidden" name="user_id" value="{{ $user->id }}">
    <input type="hidden" name="selectedDate" id="selectedDate" value="{{ old('selectedDate') }}">
    <input type="hidden" name="selectedTime" id="selectedTime" value="{{ old('selectedTime') }}">
...
@if(old('selectedDate') && old('selectedTime'))
    <div class="mt-4">
        <h4>Jadwal yang Anda pilih:</h4>
        <p>Tanggal: {{ old('selectedDate') }}</p>
        <p>Waktu: {{ old('selectedTime') }}</p>
    </div>
@endif
